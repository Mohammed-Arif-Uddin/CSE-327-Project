...
const km = 10; // Increase the range to cover all of Bangladesh

// new part
app.get('/getnearby', async (req, res) => {
  const lat = parseFloat(req.query.lat); // Latitude from the request
  const lon = parseFloat(req.query.lon); // Longitude from the request

  try {
    const posts = await Post.find({
      lat: { $gte: lat - km, $lte: lat + km }, // Approximate latitude range within 10 km
      long: { $gte: lon - km, $lte: lon + km }, // Approximate longitude range within 10 km
    });
